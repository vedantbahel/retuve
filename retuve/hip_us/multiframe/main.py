...
def get_3d_metrics_and_visuals(
    ...
    ulogger.info(f"Mesh and Sphere Creation: {time.time() - start} ms")
    ...
    ulogger.info(
        f"ACA, Normals, and Centering Ratio: {time.time() - start} ms"
    )
    ...
    if illium_mesh and femoral_sphere:
        ...
        ulogger.info(f"3D Visual Creation: {time.time() - start} ms")
    ...
    for name in config.hip.measurements:
        if not any(metric.name == name for metric in hip_datas.metrics):
            ...
            hip_datas.metrics.append(
                Metric3D(
                    name=name,
                    graf=hip_datas.grafs_hip.get_metric(name),
...
def find_graf_plane(hip_datas: HipDatasUS) -> HipDatasUS:
    """
    ...
    # find all indexes with the max alpha
    max_alpha_indexes = [
        hip_data.frame_no
        for hip_data in any_good_graf_frames
        if hip_data.get_metric(MetricUS.ALPHA)
        == graf_hip.get_metric(MetricUS.ALPHA)
    ]

    if len(max_alpha_indexes) > 1:
        hip_datas.recorded_error.append(
            f"Multiple Graf Frames: {max_alpha_indexes} "
        )
